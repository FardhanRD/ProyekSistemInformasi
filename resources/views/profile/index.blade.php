...
@section('title','MOVR | Profile')

@section('content')
<div class="space-y-6" x-data="profileTabs()">

    <div>
        <div class="text-xs font-semibold text-cyan-300">PROFILE</div>
        <h1 class="text-2xl md:text-3xl font-black">Akun Kamu</h1>
    </div>

    <div class="rounded-3xl border border-white/10 bg-white/5 p-3">
        <div class="flex flex-wrap gap-2">
            <button type="button" class="px-4 py-2 rounded-2xl text-sm font-bold border border-white/10 bg-black/20" :class="tab==='diri'?'bg-cyan-500 text-slate-950 border-cyan-400':'text-white'" @click="tab='diri'">Data Diri</button>
            <button type="button" class="px-4 py-2 rounded-2xl text-sm font-bold border border-white/10 bg-black/20" :class="tab==='alamat'?'bg-cyan-500 text-slate-950 border-cyan-400':'text-white'" @click="tab='alamat'">Alamat Saya</button>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div class="lg:col-span-8" x-show="tab==='diri'" x-cloak>
            <div class="rounded-3xl border border-white/10 bg-white/5 p-6">
                ...
                <form method="post" action="{{ route('profile.update') }}" class="mt-4 space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select name="jenis_kelamin" class="mt-2 w-full rounded-2xl border border-white/10 bg-black/20 px-4 py-2 text-sm outline-none focus:border-cyan-400">
                                <option value="L" {{ (Auth::user()->jenis_kelamin ?? '')==='L'?'selected':'' }}>L</option>
                                <option value="P" {{ (Auth::user()->jenis_kelamin ?? '')==='P'?'selected':'' }}>P</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <button type="submit" class="w-full rounded-3xl bg-cyan-500 px-6 py-3 text-sm font-bold text-slate-950 hover:bg-cyan-400">Simpan Perubahan</button>
                </form>
            </div>
        </div>
        ...
        <div class="lg:col-span-8" x-show="tab==='alamat'" x-cloak>
            <div class="rounded-3xl border border-white/10 bg-white/5 p-6">
                <h2 class="font-bold text-lg">Alamat Saya</h2>
                        ...
                        <div class="rounded-2xl border border-white/10 bg-black/20 p-4">
                            <div class="flex items-start justify-between gap-3">
                                ...
                                <div class="flex flex-col gap-2">
                                    <a href="{{ route('profile.alamat.edit', $a->alamat_id) }}" class="rounded-2xl border border-white/10 px-4 py-2 text-sm font-bold hover:bg-white/5">Edit</a>
                                    <form method="post" action="{{ route('alamat.utama', $a->alamat_id) }}">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit" class="rounded-2xl bg-cyan-500 px-4 py-2 text-sm font-bold text-slate-950 hover:bg-cyan-400">Jadikan Utama</button>
                                    </form>
                                </div>
                            </div>
                        </div>
                ...
                <div class="mt-5">
                    <a href="{{ route('profile.alamat.create') }}" class="inline-flex rounded-full bg-white/5 border border-white/10 px-5 py-3 text-sm font-bold hover:bg-white/10">Tambah Alamat</a>
                </div>
            </div>
        </div>
