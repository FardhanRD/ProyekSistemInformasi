...
@section('title','MOVR | Wishlist')

@section('content')
<div class="space-y-6">
    <div>
        <div class="text-xs font-semibold text-cyan-300">WISHLIST</div>
        <h1 class="text-2xl md:text-3xl font-black">Favorit Kamu</h1>
    </div>

    @if(empty($items) || $items->isEmpty())
        <div class="rounded-3xl border border-white/10 bg-white/5 p-10 text-center">
            <div class="text-6xl">♡</div>
            <div class="mt-3 text-lg font-bold">Wishlist kamu masih kosong</div>
            <div class="text-slate-300 text-sm mt-2">Simpan produk favorit untuk dibeli kapan saja.</div>
            <a href="{{ url('/') }}" class="mt-6 inline-flex rounded-full bg-cyan-500 px-6 py-3 text-sm font-bold text-slate-950 hover:bg-cyan-400">Cari Produk</a>
        </div>
    @else
                ...
                <div class="group rounded-3xl border border-white/10 bg-white/5 hover:bg-white/10 transition overflow-hidden">
                    <div class="relative">
                        ...
                        <form method="post" action="{{ route('wishlist.toggle') }}" class="absolute top-3 right-3">
                            ...
                            <button type="submit" class="rounded-full border border-white/15 bg-black/30 backdrop-blur px-3 py-2 text-sm hover:bg-black/40">✕</button>
                        </form>
