    ...
    <div class="relative mx-auto w-full max-w-xl px-4 sm:px-6">
        <div class="rounded-3xl border border-[#63a2bb]/25 bg-white p-8 shadow-[0_20px_60px_-24px_rgba(99,162,187,0.45)] sm:p-10">
            <div class="text-center">
                ...
                <h1 class="text-2xl font-black text-slate-800">{{ __('ui.forgot_password') }}</h1>
                <p class="mt-2 text-sm text-slate-500">{{ __('ui.forgot_password_desc') }}</p>
            </div>
            <div class="mt-8 flex flex-col gap-3 sm:flex-row">
                <a href="{{ route('login') }}" class="inline-flex flex-1 items-center justify-center rounded-xl bg-[#63a2bb] px-4 py-3 text-sm font-bold text-white transition hover:bg-[#4f90aa]">{{ __('ui.back_to_login') }}</a>
                <a href="{{ route('register') }}" class="inline-flex flex-1 items-center justify-center rounded-xl border border-slate-200 px-4 py-3 text-sm font-bold text-slate-700 transition hover:border-[#63a2bb] hover:text-[#63a2bb]">{{ __('ui.new_account') }}</a>
            </div>
        </div>
    </div>
</div>
